# Brownfield Agent — casepilot

**Run ID:** n9796x8b9718vmgqd8gckynben83s1an
**Project ID:** jd786r9d90g508cewfh9cdkf9583rbg4
**Repo:** /Users/damonbodine/Factory512/builds/casepilot-20260325-232234/casepilot
**Mode:** feature
**Request:** Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Risk Assessment AI — on client detail page, analyze intake data (demographics, housing, employment, health, support network) and generate risk score 1-10 with reasoning. Show as card with score badge + expandable reasoning. 2) Case Note Summarizer — on case notes tab, button to select date range and generate narrative summary with sections: Overview, Key Events, Service Delivery, Goals Progress, Concerns, Recommendations. 3) Service Recommendation Engine — on client detail page, analyze client profile + needs + available services, suggest top 3-5 services with match reasoning. 4) Smart Case Assignment — when creating a case, AI suggests best case worker based on caseload, specializations, and area. Show suggestion in assignment dropdown. Each feature needs: Convex action in convex/ai.ts, React component with loading/error states, proper integration into existing pages.
...
5. Work ONLY inside the target repository at /Users/damonbodine/Factory512/builds/casepilot-20260325-232234/casepilot. Never create files outside it.
...
**Query ready steps:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getReadySteps '{"runId":"n9796x8b9718vmgqd8gckynben83s1an"}'
```

**Get run status (for resume):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getRunStatus '{"runId":"n9796x8b9718vmgqd8gckynben83s1an"}'
```

**Load artifacts by kind:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getArtifactsByKinds '{"runId":"n9796x8b9718vmgqd8gckynben83s1an","kinds":["kind1","kind2"]}'
```

**Push artifact (marks step completed):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepWithArtifact '{"runId":"n9796x8b9718vmgqd8gckynben83s1an","stepNumber":N,"artifactKind":"kind","artifactContent":"<json-string>","contentType":"json"}'
```

**Complete step with no artifact:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepNoArtifact '{"runId":"n9796x8b9718vmgqd8gckynben83s1an","stepNumber":N}'
```
...
### INITIALIZE
1. `cd` to `/Users/damonbodine/Factory512/builds/casepilot-20260325-232234/casepilot`.
...
Parse `Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Risk Assessment AI — on client detail page, analyze intake data (demographics, housing, employment, health, support network) and generate risk score 1-10 with reasoning. Show as card with score badge + expandable reasoning. 2) Case Note Summarizer — on case notes tab, button to select date range and generate narrative summary with sections: Overview, Key Events, Service Delivery, Goals Progress, Concerns, Recommendations. 3) Service Recommendation Engine — on client detail page, analyze client profile + needs + available services, suggest top 3-5 services with match reasoning. 4) Smart Case Assignment — when creating a case, AI suggests best case worker based on caseload, specializations, and area. Show suggestion in assignment dropdown. Each feature needs: Convex action in convex/ai.ts, React component with loading/error states, proper integration into existing pages.` against the `codebase_profile`.
...
```json
{
  "runId": "n9796x8b9718vmgqd8gckynben83s1an",
  "projectName": "casepilot",
  "mode": "feature",
  "request": "Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Risk Assessment AI — on client detail page, analyze intake data (demographics, housing, employment, health, support network) and generate risk score 1-10 with reasoning. Show as card with score badge + expandable reasoning. 2) Case Note Summarizer — on case notes tab, button to select date range and generate narrative summary with sections: Overview, Key Events, Service Delivery, Goals Progress, Concerns, Recommendations. 3) Service Recommendation Engine — on client detail page, analyze client profile + needs + available services, suggest top 3-5 services with match reasoning. 4) Smart Case Assignment — when creating a case, AI suggests best case worker based on caseload, specializations, and area. Show suggestion in assignment dropdown. Each feature needs: Convex action in convex/ai.ts, React component with loading/error states, proper integration into existing pages.",
  "summary": "...",
  "stepsCompleted": 0,
...
1. `cd /Users/damonbodine/Factory512/builds/casepilot-20260325-232234/casepilot`
